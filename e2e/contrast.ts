 ...
 *    ratio and files the node under `incomplete`. This lab paints several of
 *    its most load-bearing surfaces that way: `.cl-hero-why` is a
 *    `color-mix(in oklab, var(--accent) 6%, transparent)` wash, the QUANTUM and
 *    CLASSICAL step-log entries tint their backdrop 8%/10% toward `--quantum` /
 *    `--success`, and the winning continued-fraction row does the same at 12%.
 *    None of those is a flat colour, so their text composites onto whatever the
 *    panel behind them is painting.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab's bar charts fade
 *    each bar in from an inline `opacity: 0`, but only when reduced motion is
 *    NOT requested; the gate emulates the preference, so that branch never
 *    runs and nothing strands mid-fade.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The step log is a
 *    fixed-height `overflow-y: auto` console and the convergents table sits in
 *    an `overflow-x: auto` wrapper, so at 380px much of both is scrolled out of
 *    the frame. Content past the client box is not dimmed or partly drawn — it
 *    is absent, and asking what colour it sits on has no answer. Its rect is
 ...
 *    phasor wheels are real SVG: each draws a `<circle>` and then its hands as
 *    following `<line>` elements. Those carry no text today, but the rule is
 *    kept because it is cheap and silent when unused — where an `<svg>` does
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * into surfaces with `color-mix(in oklab, ...)` — the hero's "why it
     * matters" wash, the quantum/classical step-log tints and the winning
     * convergent row — and Chromium reports those
     ...
     *
     * A single quantum run renders a period bar per table row, up to a few
     * hundred QFT distribution bars, six phasor wheels and a convergents table,
     * and every one of them walks the same handful of ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has three disclosures: the two `.aha`
      // explainers (open by default) and `.ecc-detail` (closed by default).
      // The gate scans both the open and the closed configuration explicitly.
      ...
      // slides in only on focus. Measuring the parked copy invents a failure
     ...
     * it, the intro's `aria-hidden` call-to-action arrow and the controls hint's
     * 🔬 are measured as if their glyphs took the element's declared `color` —
     * but an emoji paints in its own multicolour, so that ratio describes
     * nothing on screen.
     */
     ...
     * lab builds its phasor SVG with `createElementNS` and sets no character
     * data on shapes, so the rule is silent here.
     */
